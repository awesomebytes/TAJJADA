...
src_prepare() {
	...
	# The official distribution archive includes lots of prebuilt binaries
	# for various platforms. We don't need them, so we delete them, to not
	# waste space in /tmp. Useful for people who use tmpfs and have little RAM.
	rm -rf ${WORKDIR}/Xonotic/[Xx]onotic* ${WORKDIR}/Xonotic/bin*

	if ! use cpu_flags_x86_sse2; then
		epatch "${FILESDIR}/sse_disable.patch"
	fi
}
...
src_install() {
	...
	if ! use cpu_flags_x86_sse2; then
		elog "You have built Xonotic without CPU_FLAGS_X86=\"sse2\". Xonotic's"
		elog "makefile/buildsystem was modified to prevent sse-related stuff"
		elog "from being built. This is unsupported by the upstream project"
		elog "(Xonotic only officially supports x86 architecture processors"
		elog "with sse2 instructions), and therefore may or may not work."
		elog "I have tested this on an ARM system and it seems to work for me,"
		elog "but you are on your own. Good luck!"
	fi
}
